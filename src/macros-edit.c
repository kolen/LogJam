...
void macros_edit_buttonpressed_add_cb(GtkButton *widget, gpointer *user_data);
void macros_edit_selchanged_cb(GtkTreeSelection *treeselection, gpointer *user_data);
...
macros_edit_window_run(GtkWindow *parent, Macros *macros)
{
  ...
  mew->macrolist = GTK_TREE_VIEW(gtk_tree_view_new_with_model(GTK_TREE_MODEL(mew->store)));
  ...
  gtk_box_pack_start(
		     GTK_BOX(box_right), mew->name = gtk_entry_new(), FALSE, FALSE, 0
    );
  ...
  gtk_container_add(GTK_CONTAINER(b), mew->value = gtk_text_view_new());
...
void
macros_edit_buttonpressed_add_cb(GtkButton *widget, gpointer *user_data)
{
  return;
}

void
macros_edit_selchanged_cb(GtkTreeSelection *treeselection, gpointer *user_data)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
  MacroItem *m;
  MacrosEditWindow *mew;

  mew = (MacrosEditWindow*) user_data;
  ...
  g_object_set(G_OBJECT(mew->name), "bound_macroitem", G_OBJECT(m));
  g_object_set(G_OBJECT(mew->value), "bound_macroitem", G_OBJECT(m));
}
